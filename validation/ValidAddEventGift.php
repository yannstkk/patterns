...
if (empty($launchingDate)) {
    $errors['launching_date'] = 'The launch collection phase date is required';
}
if (empty($preDonationDate)) {
    $errors['pre_donation_date'] = 'The pre-donation handover date is required';
}
if (empty($postDonationDate)) {
    $errors['post_donation_date'] = 'The post-donation handover date is required';
}
...
$_SESSION['reponses'] = array_merge($previousRep, [
    ...
    'about_association' => $_POST['about_association'] ?? '',
    'active_phase' => $_POST['active_phase'] ?? 'collection',
]);
$_SESSION['errors'] = $errors;
